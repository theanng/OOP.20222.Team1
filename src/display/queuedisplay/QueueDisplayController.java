...
public class QueueDisplayController extends GeneralDisplayController {
    ...
    @FXML
    public void handleGoButtonAction() throws InterruptedException {
        ...
        if (queue.isFull()) {
            ...
            queue.print();
            // thêm màu cho tail cũ
            int tail = queue.size()- 2;
            Circle tailCircle = circles[tail];
            tailCircle.setFill(Color.ORANGE);
            ...
            Text addText = texts[enqueuedIndex];
            Line addArrowline = arrowLines[enqueuedIndex-1];
            Polygon addArrowhead = arrowHeads[enqueuedIndex-1];

            fadeTransitionCircle = new FadeTransition(Duration.seconds(fadeDuration), addCircle );
            fadeTransitionCircle.setFromValue(0.0);
            fadeTransitionCircle.setToValue(1.0);

            // Hiệu ứng text
            fadeTransitionText = new FadeTransition(Duration.seconds(fadeDuration), addText);
            fadeTransitionText.setFromValue(0.0);
            fadeTransitionText.setToValue(1.0);
            fadeTransitionText.play();
            fadeTransitionCircle.play();

            fadeTransitionCircle.setOnFinished(event -> {
                addCircle.setFill(Color.SKYBLUE);
                tailCircle.setFill(Color.WHITE);

                // Điều chỉnh opacity của addCircle
                double opacityStep = 1.0 / (fadeDuration * 100); // Chia nhỏ animation thành 100 bước
                addText.setOpacity(0.0); // Đặt opacity ban đầu là 0
                addCircle.setOpacity(0.0);
                // Tạo Timeline để tăng dần opacity của addText
                Timeline timeline = new Timeline();
                for (int i = 1; i <= 100; i++) {
                    double opacity = i * opacityStep;
                    KeyFrame keyFrameCircle = new KeyFrame(Duration.millis(i * 10), new KeyValue(addCircle.opacityProperty(), opacity));
                    KeyFrame keyFrameText = new KeyFrame(Duration.millis(i * 10), new KeyValue(addText.opacityProperty(), opacity));
                    timeline.getKeyFrames().add(keyFrameCircle);
                    timeline.getKeyFrames().add(keyFrameText);
                }
                timeline.play();
            });
        }
    ...
    @FXML
    public void handleDequeueButtonAction() throws InterruptedException {
        drawArray(queue.toArray());
        enqueueBox.setVisible(false);
        ...
        removedCircle.setFill(Color.ORANGE);

        fadeTransitionCircle = new FadeTransition(Duration.seconds(fadeDuration), removedCircle);
        fadeTransitionCircle.setFromValue(1.0);
        fadeTransitionCircle.setToValue(0.0);

        // Hiệu ứng text
        fadeTransitionText = new FadeTransition(Duration.seconds(fadeDuration), removedText);
        fadeTransitionText.setFromValue(1.0);
        fadeTransitionText.setToValue(0.0);
        fadeTransitionText.play();
        fadeTransitionCircle.play();
        ...
        fadeTransitionCircle.setOnFinished(event -> {
            ...
            Circle headerCircle = circles[0];
            headerCircle.setFill(Color.SKYBLUE);
        });
    }
